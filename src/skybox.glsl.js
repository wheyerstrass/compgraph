...
void main() {
  vert_uv = uv;
  vert_pos = (cam_rota*obj_rota * vec4(pos, 1.0)).xyz;
...
void main() {
  ...
  //float li = 0.;
  //vec3 vert_nor = 2.*texture(samp_nor,50.*vert_uv).xyz - 1.0;
  //phong(light, vert_pos, vert_nor, li);

  //vec3 base = li*1.5*texture(samp_col,50.*vert_uv).xyz;
  vec3 base = texture(samp_cube, vert_uv).xyz;
  color = vec4(base, 1.0);
}
`
}
